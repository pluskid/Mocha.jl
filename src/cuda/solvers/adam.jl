function update_parameters!(net::Net{GPUBackend}, method::Adam,
  ...
  CuBLAS.scal(net.backend.cublas_ctx, length(m), convert(data_type, beta1), get_ptr(m), 1)
  CuBLAS.axpy(net.backend.cublas_ctx, length(m), convert(data_type, 1-beta1), get_ptr(gradient), 1, get_ptr(m), 1)
  ...
  CuBLAS.scal(net.backend.cublas_ctx, length(v), convert(data_type, beta2), get_ptr(tmp), 1)
  CuBLAS.axpy(net.backend.cublas_ctx, length(v), convert(data_type, 1-beta2), get_ptr(tmp), 1, get_ptr(v), 1)
  ...
  CuBLAS.axpy(net.backend.cublas_ctx, length(param_blob), convert(data_type, -alpha_t), get_ptr(tmp), 1, get_ptr(param_blob), 1)
