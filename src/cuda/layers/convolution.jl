...
function setup_etc(backend::GPUBackend, layer::ConvolutionLayer, dtype, width, height, channels,
  ...
  for dev=1:backend.dev_count
      ...
      conv_states.elems[dev] = CuDNNConvState(inputs_desc, outputs_desc, conv_desc, filter_desc, bias_desc,
...
function shutdown_etc(backend::GPUBackend, state::ConvolutionLayerState)
  conv_states = state.etc
  for etc in conv_states.elems
...
function forward(backend::GPUBackend, state::ConvolutionLayerState, inputs::Vector{Blob})
  ...
  fwd_algorithm = etc.fwd_algorithm
  workspace_ptr = etc.workspace
  workspace_size = etc.workspace_size

  for i = 1:length(inputs)
    for g = 1:state.layer.n_group
      input_ptr = CudaPtr(get_ptr(inputs[i]).p + etc.bottom_offset * (g-1))
      output_ptr = CudaPtr(get_ptr(state.blobs[i]).p + etc.top_offset * (g-1))
      filter_ptr = CudaPtr(get_ptr(state.filter).p + etc.weight_offset * (g-1))
      CuDNN.convolution_forward(get_cudnn_ctx(backend), alpha, etc.inputs_desc[i], input_ptr,
          etc.filter_desc, filter_ptr, etc.conv_desc[i],
          etc.outputs_desc[i], output_ptr, workspace_ptr, workspace_size, fwd_algorithm,
          beta_dont_accumulate)

      # bias
      CuDNN.add_tensor4d(get_cudnn_ctx(backend), CuDNN.CUDNN_ADD_SAME_C, alpha,
          etc.bias_desc, CudaPtr(get_ptr(state.bias).p + etc.bias_offset * (g-1)),
          beta_accumulate, etc.outputs_desc[i], output_ptr)
    end
  end
end
    ...
    for g = 1:state.layer.n_group
      if !state.frozen
        ...
        CuDNN.convolution_backward_bias(get_cudnn_ctx(backend), alpha,
            etc.outputs_desc[i], CudaPtr(get_ptr(top_diff).p + etc.top_offset * (g-1)),
            beta_accumulate, etc.bias_desc, CudaPtr(get_ptr(state.∇bias).p + etc.bias_offset * (g-1)))

        # gradient w.r.t. weights
        CuDNN.convolution_backward_filter(get_cudnn_ctx(backend), alpha,
            etc.inputs_desc[i], CudaPtr(get_ptr(bottom).p + etc.bottom_offset * (g-1)),
            etc.outputs_desc[i], CudaPtr(get_ptr(top_diff).p + etc.top_offset * (g-1)),
            etc.conv_desc[i],
            beta_accumulate, etc.filter_desc, CudaPtr(get_ptr(state.∇filter).p + etc.weight_offset * (g-1)))
      end

      # gradient w.r.t. bottom data
      if isa(diffs[i], CuTensorBlob)
        CuDNN.convolution_backward_data(get_cudnn_ctx(backend), alpha,
            etc.filter_desc, CudaPtr(get_ptr(state.filter).p + etc.weight_offset * (g-1)),
            etc.outputs_desc[i], CudaPtr(get_ptr(top_diff).p + etc.top_offset * (g-1)),
            etc.conv_desc[i],
            beta_dont_accumulate, etc.inputs_desc[i], CudaPtr(get_ptr(diffs[i]).p + etc.bottom_offset * (g-1)))
      end
    end
  end
end
