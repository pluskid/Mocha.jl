const IM2COL_FLOAT_HANDLE = dlsym(Native.library, :im2col_float)
const IM2COL_DOUBLE_HANDLE = dlsym(Native.library, :im2col_double)
const COL2IM_FLOAT_HANDLE = dlsym(Native.library, :col2im_float)
const COL2IM_DOUBLE_HANDLE = dlsym(Native.library, :col2im_double)
...
function im2col{T}(img::Array{T}, n::Int, col::Array{T}, width::Int, height::Int, channels::Int,
  ...
  img_ptr = convert(Ptr{T}, img) + (width*height*channels) * (n-1) * sizeof(T)
  col_ptr = convert(Ptr{T}, col)
